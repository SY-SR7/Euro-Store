  ...
  return (
    <main className="min-h-screen bg-[#FAF7EF] px-4 py-12" dir={isAr ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-2xl space-y-6">
        <div>
          <Link href="/" className="text-xs text-[#C9A84C] hover:underline">{t('home')}</Link>
          <p className="mt-3 text-xs font-bold uppercase tracking-widest text-[#C9A84C]">{t('rewardsProgram')}</p>
        ...
        {user ? (
          <div className="rounded-3xl border border-[#E8DCC3] bg-white p-6 shadow-sm text-center space-y-3">
            ...
            <p className="text-xs text-[#A8A29E]">
              {t('equalsTo')} <strong className="text-[#C9A84C]">{pointValueSyp.toLocaleString(locale === 'ar' ? 'ar-SY' : 'en-US')} {t('syp')}</strong> {t('discountWhenShopping')}
            </p>
            {referral_code && (
            ...
            <div className="flex items-start gap-4 rounded-2xl bg-[#FAFAF8] p-4">
              ...
              <div>
                ...
                <p className="text-[#6F6658] mt-1">
                  {t('forEvery')} <strong className="text-[#C9A84C]">{earnExample.toLocaleString(locale === 'ar' ? 'ar-SY' : 'en-US')} {t('syp')}</strong> {t('youSpendYouGet')} <strong className="text-[#C9A84C]">{earnPts}</strong> {earnPts === 1 ? t('point') : t('points')}
                </p>
              </div>
            </div>
            <div className="flex items-start gap-4 rounded-2xl bg-[#FAFAF8] p-4">
