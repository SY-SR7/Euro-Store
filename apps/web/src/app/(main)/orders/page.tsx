    ...
    <main className="min-h-screen bg-background px-4 py-10" dir={isAr ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-2xl space-y-5">
        ...
        {(!orders || orders.length === 0) ? (
          <div className="rounded-2xl border border-border bg-background-card p-10 text-center shadow-sm">
            <p className="text-text-muted">{t('noOrders')}</p>
            <Link href="/products" className="mt-4 inline-block rounded-xl bg-primary px-5 py-2.5 text-sm font-bold text-[#1F1B16] hover:bg-[#9A7209] transition-colors">
              {t('browseProducts')}
            </Link>
          </div>
        ) : (
            ...
            <div className="divide-y divide-[#F0ECE6]">
              {orders.map((o: any) => (
                <Link key={o.id} href={`/orders/${o.order_number}`}
                  ...
                  <div>
                    <p className="font-mono text-sm font-bold text-text-primary">#{o.order_number}</p>
                    <p className="mt-0.5 text-xs text-text-muted">{new Date(o.created_at).toLocaleDateString(locale === 'ar' ? 'ar-SY' : 'en-US')}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <p className="text-sm font-semibold text-text-secondary">{Number(o.total_syp).toLocaleString(locale === 'ar' ? 'ar-SY' : 'en-US')} {t('syp')}</p>
