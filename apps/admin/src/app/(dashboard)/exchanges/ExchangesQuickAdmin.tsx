...
function Modal({ title, onClose, children, closeTitle }: { title: string; onClose: () => void; children: ReactNode; closeTitle?: string }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3" onClick={onClose}>
      <div
        className="flex max-h-[85dvh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-[#E5E0D8] bg-[#FFFCF7] shadow-2xl"
    ...
    <div className="space-y-5" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="overflow-hidden rounded-2xl border border-[#E5E0D8] bg-background-card shadow-sm">
        {loading ? (
          ...
          <p className="p-4 md:p-10 text-center text-sm text-text-muted">{t('noExchanges', { fallback: 'لا توجد طلبات استبدال' })}</p>
        ) : (
