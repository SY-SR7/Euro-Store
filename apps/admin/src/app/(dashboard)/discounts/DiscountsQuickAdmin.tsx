...
function InlineNumber({ value, onSave, nullable = false }: { value?: number | null; onSave: (value: number | null) => void | Promise<void>; nullable?: boolean }) {
  const [editing, setEditing] = useState(false);
  ...
  return <button type="button" onClick={() => setEditing(true)} className="min-h-9 w-full rounded-xl px-3 py-2 text-start text-sm font-bold text-[#1C1917] transition hover:bg-[#FAF7EF]">{value == null ? <span className="text-[#A8A29E]">—</span> : Number(value).toLocaleString('ar-SY')}</button>;
}

function TypePills({ value, onSave }: { value: string; onSave: (value: string) => void | Promise<void> }) {
  return (
    <div className="flex gap-2">
      {[['percentage', 'نسبة %'], ['fixed', 'مبلغ ثابت']].map(([key, label]) => (
    ...
    <div className="space-y-5" dir="rtl">
      <div className="flex flex-col gap-4 rounded-2xl border border-[#E5E0D8] bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center gap-2 rounded-xl bg-[#1C1917] px-4 py-2 text-sm font-black text-white hover:bg-[#2D2926]"><Plus size={15} />كود جديد</button>
        </div>
      </div>
      ...
      {showCreate ? (
        <div className="rounded-2xl border border-[#E5E0D8] bg-white p-5 shadow-sm">
          <div className="grid gap-3 sm:grid-cols-2">
            <input value={newForm.code} onChange={(e) => setNewForm((f) => ({ ...f, code: e.target.value.toUpperCase() }))} placeholder="الكود" className={`${inputClass} font-mono`} dir="ltr" />
            <select value={newForm.type} onChange={(e) => setNewForm((f) => ({ ...f, type: e.target.value }))} className={inputClass}>
              <option value="percentage">نسبة %</option>
              <option value="fixed">مبلغ ثابت</option>
            </select>
            <input type="number" value={newForm.value} onChange={(e) => setNewForm((f) => ({ ...f, value: e.target.value }))} placeholder="القيمة" className={inputClass} />
            <input type="number" value={newForm.min_order_syp} onChange={(e) => setNewForm((f) => ({ ...f, min_order_syp: e.target.value }))} placeholder="الحد الأدنى" className={inputClass} />
            ...
            <input type="number" value={newForm.max_uses} onChange={(e) => setNewForm((f) => ({ ...f, max_uses: e.target.value }))} placeholder="أقصى استخدام" className={inputClass} />
            <button type="button" onClick={() => void createDiscount()} disabled={!newForm.code || !newForm.value} className="rounded-xl bg-[#B8860B] py-2 text-sm font-bold text-white disabled:opacity-50">إنشاء</button>
          </div>
        </div>
      ) : null}
                ...
                {discounts.map((discount) => (
                  <tr key={discount.id} className="group cursor-pointer transition-colors hover:bg-[#FFFBF0]" onClick={() => openDiscount(discount)}>
                    ...
                    <td className="hidden px-5 py-3 md:table-cell" onClick={(e) => e.stopPropagation()}>
                      <button type="button" onClick={() => void patchDiscount(discount, { is_active: !discount.is_active })} className={`rounded-full border px-3 py-1 text-xs font-bold ${discount.is_active ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'}`}>{discount.is_active ? 'نشط' : 'معطّل'}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-[#E5E0D8] bg-white p-4 shadow-sm">
              <div className="space-y-2">
                <Field label="الكود"><InlineText value={selected.code} dir="ltr" onSave={(code) => patchDiscount(selected, { code: code.toUpperCase() })} /></Field>
                <Field label="النوع"><TypePills value={selected.type} onSave={(type) => patchDiscount(selected, { type })} /></Field>
                <Field label="القيمة"><InlineNumber value={selected.value} onSave={(value) => patchDiscount(selected, { value: Number(value ?? 0) })} /></Field>
                <Field label="الحد الأدنى"><InlineNumber value={selected.min_order_syp ?? null} nullable onSave={(value) => patchDiscount(selected, { min_order_syp: value })} /></Field>
                <Field label="أقصى استخدام"><InlineNumber value={selected.max_uses ?? null} nullable onSave={(value) => patchDiscount(selected, { max_uses: value })} /></Field>
                <Field label="صالح من"><input type="date" value={toDateInput(selected.valid_from)} onChange={(e) => void patchDiscount(selected, { valid_from: e.target.value || null })} className={inputClass} dir="ltr" /></Field>
                <Field label="صالح حتى"><input type="date" value={toDateInput(selected.valid_until)} onChange={(e) => void patchDiscount(selected, { valid_until: e.target.value || null })} className={inputClass} dir="ltr" /></Field>
                <Field label="الحالة"><ActivePills value={selected.is_active} onSave={(is_active) => patchDiscount(selected, { is_active })} /></Field>
                <Field label="الاستخدامات"><div className="min-h-9 rounded-xl px-3 py-2 text-sm font-semibold text-[#1C1917]">{selected.used_count ?? 0}{selected.max_uses ? ` / ${selected.max_uses}` : ''}</div></Field>
                <Field label="النطاق"><div className="min-h-9 rounded-xl px-3 py-2 text-sm font-semibold text-[#1C1917]">{formatDate(selected.valid_from)} - {formatDate(selected.valid_until)}</div></Field>
              </div>
            </div>
          </div>
        </Modal>
