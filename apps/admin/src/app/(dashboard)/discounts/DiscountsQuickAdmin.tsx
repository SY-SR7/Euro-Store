  ...
  return (
    <div className="space-y-5" dir={isAr ? "rtl" : "ltr"}>
      <div className="flex flex-col gap-4 rounded-2xl border border-[#E5E0D8] bg-background-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-black text-text-primary">{t('discountsTitle', { fallback: 'الخصومات' })}</h1>
          <p className="mt-1 text-sm text-text-muted">{t('discountsCount', { count: discounts.length, fallback: `${discounts.length} كود خصم` })}</p>
        </div>
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-black text-text-primary hover:bg-primary-dark"><Plus size={15} />{t('newDiscount', { fallback: 'كود جديد' })}</button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[#E5E0D8] bg-background-card shadow-sm">
        ...
        : discounts.length === 0 ? <p className="p-4 md:p-10 text-center text-sm text-text-muted">{t('noDiscounts', { fallback: 'لا توجد أكواد خصم' })}</p>
        : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-[#F8F6F2]"><tr>{[t('code', { fallback: 'الكود' }), t('type', { fallback: 'النوع' }), t('value', { fallback: 'القيمة' }), t('uses', { fallback: 'الاستخدامات' }), t('status', { fallback: 'الحالة' })].map((h,i)=><th key={h} className={`px-5 py-3 ${isAr ? "text-right" : "text-left"} text-xs font-black text-text-muted ${i>=3?'hidden md:table-cell':''}`}>{h}</th>)}</tr></thead>
              <tbody className="divide-y divide-[#F0ECE6]">
                {discounts.map((discount) => (
                  <tr key={discount.id} className="group cursor-pointer transition-colors hover:bg-[#FFFBF0]" onClick={() => openDiscount(discount)}>
                    <td className="px-5 py-3 font-mono font-bold text-text-primary group-hover:text-primary">{discount.code}</td>
                    <td className="px-5 py-3 text-text-secondary">{discount.type === 'percentage' ? t('percentage', { fallback: 'نسبة' }) : t('fixed', { fallback: 'ثابت' })}</td>
                    <td className="px-5 py-3 font-bold text-primary">{discount.type === 'percentage' ? `${discount.value}%` : money(discount.value, locale === 'ar' ? 'ar-SY' : 'en-US')}</td>
                    <td className="hidden px-5 py-3 text-xs text-text-muted md:table-cell">{discount.used_count ?? 0}{discount.max_uses ? ` / ${discount.max_uses}` : ''}</td>
