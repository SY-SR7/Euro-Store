  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between rounded-3xl border border-[#E8DCC3] bg-[#FFFDF8] p-6">
        <div>
          <Link href={`/products/${product.id}`}
            className="text-xs text-[#6F6658] hover:text-[#C9A84C] transition-colors">
            ← {product.name_ar}
          </Link>
          <h1 className="mt-1 text-2xl font-black text-[#1F1B16]">إدارة الصور</h1>
          <p className="mt-1 text-sm text-[#6F6658]">{(images ?? []).length} صورة مضافة</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        {/* Current images grid */}
        <div className="rounded-3xl border border-[#E8DCC3] bg-[#FFFDF8] p-6">
          <h2 className="mb-4 text-base font-black text-[#C9A84C]">الصور الحالية</h2>
          {(!images || images.length === 0) ? (
            <div className="rounded-2xl border border-dashed border-[#E8DCC3] p-12 text-center text-[#6F6658]">
              لا توجد صور بعد — ارفع أول صورة من القسم الجانبي
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {images.map((img: any) => (
                <div key={img.id} className="group relative overflow-hidden rounded-2xl border border-[#E8DCC3] bg-[#FFFDF8]">
