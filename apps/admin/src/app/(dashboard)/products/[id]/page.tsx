...
export default function ProductHubPage() {
  ...
  const [newVariant, setNewVariant]     = useState({ sku:'', price_syp:'', compare_price_syp:'', stock_quantity:'0', color:'', size:'', is_active:true });
  ...
  const load = useCallback(async () => {
    ...
    const [prodRes, varRes, imgRes, catRes, brRes] = await Promise.all([
  ...
  const saveVariant = async () => {
    ...
    if (variantDraft.color !== undefined)          body.color = variantDraft.color;
    if (variantDraft.size !== undefined)           body.size = variantDraft.size;
    ...
    if (res.ok) { setMsg('✓ تم حفظ المتغير'); setEditVariant(null); await load(); }
    else { const d = await res.json().catch(()=>null); setMsg('✗ ' + (d?.error ?? 'فشل')); }
    setSaving(false);
  };
  ...
  const createVariant = async () => {
    setSaving(true); setMsg('');
    const body: any = {
      product_id: id, sku: newVariant.sku,
    ...
    if (newVariant.color) body.color = newVariant.color;
    if (newVariant.size)  body.size  = newVariant.size;
    const res = await fetch('/api/catalog/variants', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
    });
    if (res.ok) { setMsg('✓ تم إضافة المتغير'); setAddVariant(false); setNewVariant({ sku:'', price_syp:'', compare_price_syp:'', stock_quantity:'0', color:'', size:'', is_active:true }); await load(); }
    else { const d = await res.json().catch(()=>null); setMsg('✗ ' + (d?.error ?? 'فشل')); }
    setSaving(false);
  };
  ...
  const deleteVariant = async (v: any) => {
    if (!confirm(`حذف المتغير "${v.sku}"؟`)) return;
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          { label:'المتغيرات', value: variants.length },
      ...
      <div className="rounded-2xl border border-[#E5E0D8] bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-[#F0ECE6]">
          <h2 className="font-black text-[#1C1917]">المتغيرات والأسعار ({variants.length})</h2>
          <button onClick={() => { setAddVariant(true); setMsg(''); }}
            className="rounded-xl bg-[#B8860B] px-4 py-2 text-xs font-bold text-white hover:bg-[#9A7209]">
            + متغير جديد
          </button>
        </div>
        {variants.length === 0 ? (
          <p className="p-8 text-center text-sm text-[#A8A29E]">لا توجد متغيرات — أضف متغيراً لتحديد السعر والمخزون</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-[#F8F6F2]">
                <tr>
                  {['SKU','اللون','المقاس','السعر (ل.س)','مقارنة','المخزون','الحالة','إجراء'].map((h,i) => (
              ...
              <tbody className="divide-y divide-[#F0ECE6]">
                {variants.map((v: any) => (
                  <tr key={v.id} className="hover:bg-[#FAFAF8] cursor-pointer" onClick={() => { setEditVariant(v); setVariantDraft({ ...v }); setMsg(''); }}>
                    <td className="px-4 py-3 font-mono text-xs text-[#57534E]">{v.sku}</td>
                    <td className="px-4 py-3 text-xs text-[#57534E]">{v.color ?? '—'}</td>
                    <td className="px-4 py-3 text-xs text-[#57534E]">{v.size ?? '—'}</td>
      ...
      <div className="rounded-2xl border border-[#E5E0D8] bg-white p-5 shadow-sm space-y-4">
        ...
        {images.length === 0 ? (
          <p className="text-sm text-[#A8A29E] text-center py-4">لا توجد صور — أضف رابط صورة للمنتج</p>
        ) : (
      ...
      {editProduct && (
        <Modal title="تعديل المنتج" onClose={() => setEditProduct(false)}>
          <div className="space-y-3">
            {[['name_ar','الاسم بالعربية'],['name_en','الاسم بالإنجليزية'],['slug','Slug (روابط فقط)']].map(([k,lbl]) => (
              <div key={k}>
      ...
      {/* ── MODAL: Edit Variant ── */}
      {editVariant && (
        <Modal title={`تعديل المتغير: ${editVariant.sku}`} onClose={() => setEditVariant(null)}>
          <div className="space-y-3">
            {[['sku','SKU'],['price_syp','السعر (ل.س)'],['compare_price_syp','سعر المقارنة (ل.س)'],['stock_quantity','المخزون'],['color','اللون'],['size','المقاس']].map(([k,lbl]) => (
              <div key={k}>
                <label className="mb-1 block text-xs font-bold text-[#A8A29E]">{lbl}</label>
                <input
                  type={['price_syp','compare_price_syp','stock_quantity'].includes(k) ? 'number' : 'text'}
      ...
      {/* ── MODAL: Add Variant ── */}
      {addVariant && (
        <Modal title="إضافة متغير جديد" onClose={() => setAddVariant(false)}>
          <div className="space-y-3">
            {[['sku','SKU *'],['price_syp','السعر (ل.س) *'],['compare_price_syp','سعر المقارنة (ل.س)'],['stock_quantity','المخزون'],['color','اللون (اختياري)'],['size','المقاس (اختياري)']].map(([k,lbl]) => (
              <div key={k}>
                <label className="mb-1 block text-xs font-bold text-[#A8A29E]">{lbl}</label>
                <input
                  type={['price_syp','compare_price_syp','stock_quantity'].includes(k) ? 'number' : 'text'}
