    ...
    <div className="space-y-5" dir={isAr ? "rtl" : "ltr"}>
      <div className="flex flex-col gap-4 rounded-2xl border border-[#E5E0D8] bg-background-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-black text-text-primary hover:bg-primary-dark"><Plus size={15} />{t('newBrand', { fallback: 'علامة تجارية جديدة' })}</button>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-[#E5E0D8] bg-background-card p-4 shadow-sm">
              <div className="space-y-2">
                ...
                <Field label={t('primaryImage', { fallback: 'الشعار' })}><InlineImage value={selected.logo_url ?? ''} dir="ltr" onSave={(logo_url) => patchBrand(selected, { logo_url })} /></Field>
