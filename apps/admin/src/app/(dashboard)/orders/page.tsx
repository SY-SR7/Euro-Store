...
const STATUS_AR: Record<string,string> = {
  pending:'قيد الانتظار', confirmed:'مؤكد', processing:'جارٍ التجهيز',
...
function LegacyAdminOrdersPage() {
  ...
  const changeStatus = async (o: Order, newStatus: string) => {
    ...
    if (res.ok) {
      ...
      setMsg(`✓ تم تغيير الحالة إلى: ${STATUS_AR[newStatus]??newStatus}`);
  ...
  return (
    <div className="space-y-5" dir="rtl">
      <div className="flex items-center justify-between rounded-2xl border border-[#E5E0D8] bg-white p-5 shadow-sm">
        <div><h1 className="text-2xl font-black text-[#1C1917]">الطلبات</h1><p className="mt-1 text-sm text-[#A8A29E]">{total} إجمالي الطلبات</p></div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-[#F8F6F2]"><tr>
                {['رقم الطلب','العميل','الحالة','المجموع','التاريخ'].map((h,i)=>(
      ...
      {selected&&(
        <Modal title={`طلب #${selected.order_number}`} onClose={()=>setSelected(null)}>
          ...
          <div className="space-y-3 text-sm">
            {([['العميل',selected.address_snapshot?.full_name],['الهاتف',selected.address_snapshot?.phone],['المحافظة',selected.address_snapshot?.governorate],['المجموع',`${Number(selected.total_syp).toLocaleString('ar-SY')} ل.س`],['التاريخ',new Date(selected.created_at).toLocaleDateString('ar-SY')]] as [string,string|undefined][]).map(([l,v])=>(
