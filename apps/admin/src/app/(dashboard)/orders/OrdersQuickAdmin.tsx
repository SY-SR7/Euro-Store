...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3" onClick={onClose}>
      <div
        className="flex max-h-[85dvh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-[#E5E0D8] bg-[#FFFCF7] shadow-2xl"
  ...
  return (
    <div className="space-y-5" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="overflow-hidden rounded-2xl border border-[#E5E0D8] bg-background-card shadow-sm">
        {loading ? (
          <p className="p-4 md:p-10 text-center text-sm text-text-muted">{t('loading', { fallback: 'جار التحميل...' })}</p>
        ) : orders.length === 0 ? (
          <p className="p-4 md:p-10 text-center text-sm text-text-muted">{t('noOrders', { fallback: 'لا توجد طلبات' })}</p>
        ) : (
