...
export default function AdminSubAdminsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (res.ok) {
      setMsg('تم إنشاء حساب المسؤول الفرعي بنجاح');
      setShowForm(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          {showForm ? t('common.cancel') : '+ إضافة مسؤول فرعي'}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={(e) => void handleCreate(e)} className="rounded-lg border border-[#2E2E2E] bg-[#111] p-6 space-y-4">
          <h2 className="font-semibold text-[#E2E2E2] mb-2">إنشاء حساب مسؤول فرعي جديد</h2>
          {err && <p className="text-sm text-red-400">{err}</p>}
          <div>
            <label className="block text-xs text-[#9CA3AF] mb-1">الاسم الظاهر</label>
            <input className={inputCls} value={form.display_name} onChange={e => setForm(f => ({ ...f, display_name: e.target.value }))} placeholder="اسم المسؤول" required />
          </div>
          <div>
            <label className="block text-xs text-[#9CA3AF] mb-1">البريد الإلكتروني</label>
            <input className={inputCls} type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="admin@example.com" required />
          </div>
          <div>
            <label className="block text-xs text-[#9CA3AF] mb-1">كلمة المرور (8 أحرف على الأقل)</label>
            <input className={inputCls} type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" minLength={8} required />
          </div>
          <button
            type="submit"
            disabled={creating}
      ...
      {loading ? (
        ...
        <div className="rounded-md border border-[#2E2E2E] bg-[#151515] p-8 text-center text-[#9CA3AF]">
          لا يوجد مسؤولون فرعيون حتى الآن
        </div>
      ) : (
