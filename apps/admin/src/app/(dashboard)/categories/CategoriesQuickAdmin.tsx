...
function InlineImage({ value, onSave, dir = 'ltr' }: { value?: string | null; onSave: (value: string) => void | Promise<void>; dir?: 'rtl' | 'ltr' }) {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      const res = await fetch('/api/upload?purpose=category', { method: 'POST', body: formData });
    ...
    return (
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <input 
            aria-label="وسائط التصنيف / Category media"
            type="file" 
            accept="image/*,video/*"
...
function InlineNumber({ value, onSave }: { value?: number | null; onSave: (value: number) => void | Promise<void> }) {
  ...
  if (editing) return <input aria-label="ترتيب التصنيف / Category order" autoFocus type="number" value={draft} onBlur={commit} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') setEditing(false); }} className={inputClass} />;
    ...
    <div className="space-y-5" dir={isAr ? "rtl" : "ltr"}>
      <div className="flex flex-col gap-4 rounded-2xl border border-[#E5E0D8] bg-background-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => setShowCreate((value) => !value)} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-black text-text-primary hover:bg-primary-dark"><Plus size={15} />{t('newCategory', { fallback: 'تصنيف جديد' })}</button>
        </div>
      </div>
