...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin.from('shipping_rates').select('*').eq('id', (await params).id).single();
...
const { admin } = ctx;
  ...
  const { data, error } = await admin.from('shipping_rates').update(update).eq('id', (await params).id).select().single();
