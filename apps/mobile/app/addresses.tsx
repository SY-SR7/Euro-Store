...
export default function AddressesScreen() {
  ...
  return (
    <SafeAreaView className='flex-1 bg-background'>
      <ScreenHeader title={l('عناوين التوصيل', 'Delivery addresses')} action={<TouchableOpacity accessibilityLabel={showForm ? l('إغلاق النموذج', 'Close form') : l('إضافة عنوان', 'Add address')} onPress={() => setShowForm((value) => !value)} className='h-10 w-10 items-center justify-center rounded-full bg-primary'><Text className='text-2xl font-bold text-[#0F0F0F]'>{showForm ? '×' : '+'}</Text></TouchableOpacity>} />
      {loading ? <View className='flex-1 items-center justify-center'><ActivityIndicator size='large' color='#B8860B' /></View> : (
        <ScrollView className='flex-1 px-6' contentContainerStyle={{ paddingVertical: 24 }} keyboardShouldPersistTaps='handled'>
          {showForm ? <View className='mb-6 border-b border-border pb-6'>
            ...
            <TouchableOpacity disabled={saving} onPress={add} className={`rounded-xl bg-primary p-4 ${saving ? 'opacity-60' : ''}`}><Text className='text-center font-bold text-[#0F0F0F]'>{saving ? l('جارٍ الحفظ...', 'Saving...') : l('إضافة العنوان', 'Add address')}</Text></TouchableOpacity>
          </View> : null}
