...
import { getPartnerAccess } from './src/auth';
...
export async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  const contentSecurityPolicy = createContentSecurityPolicy('partner', nonce, process.env.NODE_ENV === 'development');
  ...
  if (pathname.startsWith('/api/')) {
    const rate = await limitApiRequest(request, apiRateLimitCategory(pathname, 'partner'));
    if (!rate.success) {
