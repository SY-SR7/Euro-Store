...
export function QRScanner({ onScan, onClose, title, description }: QRScannerProps) {
  ...
  const scanFrame = useCallback(async () => {
    ...
    try {
      ...
      if (code?.data) { setScanned(true); stopCamera(); onScan(code.data); return; }
    } catch {
      stopCamera();
      setError(t('cameraUnknownError'));
      return;
    }
    rafRef.current = requestAnimationFrame(() => { void scanFrame(); });
  }, [scanned, stopCamera, onScan, t]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#0F0F0F]">
      <div className="flex items-center justify-between px-4 py-4 border-b border-[#2E2E2E]">
        <div>
          <h2 className="text-lg font-bold text-[#E2E2E2]">{title ?? t('exchangeScanner')}</h2>
          {description && <p className="text-xs text-[#9CA3AF] mt-0.5">{description}</p>}
        </div>
        <button onClick={() => { stopCamera(); onClose(); }}
          className="rounded-full p-2 text-[#9CA3AF] hover:bg-[#1E1E1E] transition-colors" aria-label={t('close')}>
          <X size={20} />
        </button>
      </div>
      <div className="relative flex-1 flex items-center justify-center bg-black overflow-hidden">
        {error ? (
          <div className="flex flex-col items-center gap-4 px-8 text-center">
            ...
            <button onClick={() => { stopCamera(); onClose(); }}
              className="rounded-xl bg-[#1E1E1E] border border-[#2E2E2E] px-6 py-3 text-sm text-[#E2E2E2]">
              {t('back')}
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
              <div className="relative h-56 w-56">
                ...
                ].map((cls, i) => <div key={i} className={`absolute h-10 w-10 border-primary ${cls}`} />)}
              </div>
            </div>
            <div className="absolute bottom-8 left-0 right-0 flex justify-center">
              <div className="rounded-full bg-black/60 px-4 py-2 backdrop-blur-sm">
                {scanned ? <p className="text-green-400 text-sm font-bold">{t('scanComplete')}</p>
                  : ready ? <p className="text-[#9CA3AF] text-sm">{t('pointCamera')}</p>
