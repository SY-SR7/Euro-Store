...
export default function PartnerDashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0F0F0F] p-8 text-[#E2E2E2]">
      <h1 className="text-2xl font-semibold mb-6">{t('dashboardTitle')}</h1>

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-3 mb-8">
        <div className="rounded-xl border border-[#2E2E2E] bg-[#151515] p-5 text-center">
          <p className="text-4xl font-black text-primary mb-1">
            {loading ? '…' : queue.length}
          </p>
          <p className="text-sm text-[#9CA3AF]">{t('totalRequests')}</p>
        </div>
        <div className={`rounded-xl border p-5 text-center ${
          pending > 0 ? 'border-amber-700/40 bg-amber-900/15' : 'border-[#2E2E2E] bg-[#151515]'
        }`}>
          <p className={`text-4xl font-black mb-1 ${pending > 0 ? 'text-amber-400' : 'text-primary'}`}>
            {loading ? '…' : pending}
          </p>
          <p className="text-sm text-[#9CA3AF]">{t('awaitingReceipt')}</p>
        </div>
        <div className="rounded-xl border border-[#2E2E2E] bg-[#151515] p-5 text-center">
          <p className="text-4xl font-black text-green-400 mb-1">
            {loading ? '…' : inTransit}
          </p>
          <p className="text-sm text-[#9CA3AF]">{t('inProgress')}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 max-w-xl">
        <Link href="/exchange"
          className="rounded-xl border border-[#2E2E2E] bg-[#151515] p-6 hover:border-primary transition-colors">
          <ScanLine size={28} className="mb-3 text-primary" />
          <p className="font-semibold text-primary">{t('exchangeScanner')}</p>
          <p className="mt-1 text-sm text-[#9CA3AF]">{t('exchangeScannerDesc')}</p>
        </Link>
        <Link href="/exchange/history"
          ...
          <p className="mt-1 text-sm text-[#9CA3AF]">{t('exchangeHistoryDesc')}</p>
        </Link>
      </div>
    </main>
  );
