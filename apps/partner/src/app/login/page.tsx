import { loginPartnerAction } from './actions';
...
export default function PartnerLoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0F0F0F] px-6 py-16 text-[#E2E2E2]">
      <section className="mx-auto flex w-full max-w-md flex-col gap-8">
        <div>
          <p className="text-sm text-[#C9A84C]">EuroStore Partner</p>
          <h1 className="mt-3 text-3xl font-semibold">دخول الشريك</h1>
        </div>

        {message && <p className="rounded border border-[#2E2E2E] p-4 text-sm">{message}</p>}

        <form action={loginPartnerAction} className="flex flex-col gap-4">
