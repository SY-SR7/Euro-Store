...
export default async function PartnerLoginPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <main className="min-h-screen bg-[#0F0F0F] text-[#E2E2E2] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <h1 className="mt-3 text-3xl font-semibold">{t('partnerLogin')}</h1>
        {errorMsg && <p className="mt-4 rounded border border-[#2E2E2E] p-4 text-sm text-red-400">{errorMsg}</p>}
        <form action={partnerLogin} className="mt-8 flex flex-col gap-4">
