...
for (const asset of assets) {
  ...
  if (!existsSync(videoPath)) {
    ...
    try {
      execSync(`C:\\Users\\Ammar\\AppData\\Local\\Python\\pythoncore-3.14-64\\Scripts\\yt-dlp.exe "${asset.searchQuery}" -f "best[ext=mp4]" --match-filter "duration < 180" -o "${videoPath}"`, {
        cwd: rootDir,
        stdio: 'inherit',
      });
    } catch (err) {
