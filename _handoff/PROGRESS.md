...
### 21. إزالة الفراغات السفلية الزائدة وضبط تخطيط الصفحات (Eliminate Excess Bottom Spacing & Clean Flex Layout)
- **سبب المشكلة السابقة:**
  - كان هناك تكرار وتداخل لخاصية `min-h-screen` داخل `layout.tsx` ثم مرة أخرى داخل صفحات الأقسام والمنتجات (`new-arrivals`, `offers`, `products`, `categories`) مما تسبب في إضافة ارتفاعات مضاعفة غير ضرورية وخلق فراغات بيضاء وعاجية شاسعة أسفل شبكة المنتجات وأزرار الترقيم.
- **الحل المنفذ:**
  - تعديل هيكل التخطيط في `apps/web/src/app/(main)/layout.tsx` ليصبح حاوية مرنة طبيعية `flex flex-col min-h-screen` مع جعل قسم المحتوى الرئيسي `main` يأخذ الارتفاع التلقائي الطبيعي المناسب لمحتواه (`flex-1`).
  - إزالة تكرار وسوم `main` و `min-h-screen` المتداخلة من الصفحات، ليجلس الفوتر والتذييل مباشرة بعد نهاية محتوى الصفحة أو شبكة المنتجات بشكل طبيعي وانسيابي ومحكم دون أي فراغات مصطنعة.
...
## توحيد النظام البصري الفاتح وإلغاء إرث الأسود/الذهبي - 2026-08-30

- أصبح `_handoff/DESIGN.md` المرجع البصري الملزم: خلفيات عاجية دافئة، نص فحمي، وذهبي محدود كلون إبراز.
- أُلغيت كل توجيهات dark/system theme ومفاتيح حفظ الثيم من PRD والاستراتيجية والقواعد وملفات التسليم ذات الصلة.
- نُظفت المصادر المشتركة (`packages/config` و`packages/ui`) وتطبيق الهاتف والويب ولوحات admin/helper/partner من النمط الداكن القديم.
- أُعيد توليد `CODEX_UI_CONTROL_INVENTORY_2026-08-28.json` من الشفرة الحالية بدل إبقاء snapshot قديم مضلل.
- أُضيف `tests/theme-contract.test.ts` لمنع رجوع الألوان أو سلوك الثيم القديم مستقبلًا.
- التحقق: 56/56 اختبار، TypeScript ناجح لكل التطبيقات والحزم، وفحص بصري على قياس iPhone للمتجر وصفحات الدخول الإدارية.
