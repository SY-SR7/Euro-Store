...
### 20. إظهار السعر قبل وبعد الخصم بشكل راقي على كافة بطاقات المنتجات (Elegant Before/After Price & Discount Display)
- **المطلب:** عرض السعر الأصلي مشطوباً إلى جانب السعر المخفض الجديد مع نسبة الخصم على كل بطاقة منتج يوجد عليه عرض أو تخفيض.
- **التطويرات المنفذة:**
  - **ترقية مكون السعر العام `PriceDisplay`:**
    - دعم حساب وعرض السعر قبل الخصم المشطوب (`line-through`) بلون هادئ وأنيق.
    - إضافة بادج فاخر لنسبة الخصم (مثل `-15%` أو `-20%`) بجانب السعر.
    - الحفاظ على المكافئ بالدولار الأمريكي المحدث لحظياً.
  - **تحديث بطاقة المنتج `ProductCard` في الكتالوج والواجهة:**
    - إضافة بادج ذهبي فاخر على صورة المنتج في الزاوية يوضح نسبة الخصم فوراً (مثل `-15%` / `-25%`).
    - عرض سطر الأسعار بشكل واضح ومتناسق: **السعر الحالي المخفض** + **السعر القديم مشطوباً** + **شارة نسبة التوفير**.
  - **تفعيل التخفيضات الرسمية على المنتجات الرئيسية في قاعدة البيانات:**
    - ديور سوفاج (-15%): أصبح 2,175,000 ل.س بدلاً من 2,558,824 ل.س.
    - فرزاتشي إيروس (-15%): أصبح 1,725,000 ل.س بدلاً من 2,029,412 ل.س.
    - أديداس سوبر ستار (-20%): أصبح 1,725,000 ل.س بدلاً من 2,156,250 ل.س.
    - بوما سماش (-25%): أصبح 1,050,000 ل.س بدلاً من 1,400,000 ل.س.
    - وكافة منتجات العروض في كامل المتجر.
