...
### 13. إنجاز سياسة الخصوصية وشروط الاستخدام وتفعيل فلتر العروض والتخفيضات (Comprehensive Legal Pages A-Z & Sale Filtering)
- **إنشاء وتجهيز وثائق سياسة الخصوصية وشروط الاستخدام بالكامل من A إلى Z (`/privacy` و `/terms`):**
  - صياغة قانونية احترافية وشاملة بالعربية والإنجليزية تغطي: الأهلية، دقة الأسعار والمخزون، الشحن والتوصيل لكافة المحافظات السورية، ضمان الأصالة 100%، سياسة الاستبدال خلال 3 أيام، وسائل الدفع (كاش وشام كاش)، حماية البيانات والتشفير، وملفات تعريف الارتباط.
  - ربط الروابط بوضوح في تذييل الصفحة (`Footer`) ضمن قسم السياسات وشريط الحقوق السفلي.
- **تفعيل فلتر العروض والتخفيضات برمجياً (`/products?sale=true`):**
  - تطبيق خصومات موسمية رسمية بنسب (10% إلى 25%) على 12 منتجاً من أشهر الموديلات.
  - دعم فلترة المنتجات المخفضة عبر الـ API ومكون شبكة المنتجات مع إضافة زر تبديل `🏷️ العروض والتخفيضات فقط` في القائمة الجانبية للفلاتر.
