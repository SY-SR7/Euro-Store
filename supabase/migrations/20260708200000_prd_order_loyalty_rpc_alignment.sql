...
DROP FUNCTION IF EXISTS public.award_loyalty_points(UUID, INT, TEXT, TEXT, UUID);
DROP FUNCTION IF EXISTS public.award_loyalty_points(UUID, INTEGER, public.loyalty_tx_type, UUID, UUID, public.user_role, TEXT);
DROP FUNCTION IF EXISTS public.award_loyalty_points(UUID, INTEGER, public.loyalty_tx_type, UUID, UUID, public.user_role, TEXT, TEXT);

CREATE OR REPLACE FUNCTION public.award_loyalty_points(
  p_customer_id UUID,
  p_points INTEGER,
  p_type public.loyalty_tx_type,
  p_reference_id UUID DEFAULT NULL,
  p_processed_by_id UUID DEFAULT NULL,
  p_processed_by_role public.user_role DEFAULT 'helper',
...
BEGIN
  ...
  INSERT INTO public.loyalty_points_transactions (
    ...
    p_processed_by_id,
    p_processed_by_role,
    COALESCE(p_notes, p_description)
  );
END;
$$;
