...
function pickArray<T>(p:unknown):T[]{if(Array.isArray(p))return p as T[];if(p&&typeof p==='object'){const o=p as Record<string,unknown>;for(const k of['data','items','shipping_rates']){if(Array.isArray(o[k]))return o[k] as T[];}}return[];}
...
export default function AdminShippingRatesPage() {
  ...
  const load = useCallback(async()=>{
    ...
    setRates(pickArray<ShippingRate>(d)); setLoading(false);
  },[]);
  useEffect(()=>{ void load(); },[load]);

  const open = (r:ShippingRate) => {
    setSelected(r); setMsg('');
    setDraft({ base_rate_syp:String(r.base_rate_syp), free_shipping_threshold_syp:String(r.free_shipping_threshold_syp??''), is_active:r.is_active });
  };
  ...
  return (
    <div className="space-y-5" dir="rtl">
      <div className="flex items-center justify-between rounded-2xl border border-[#E5E0D8] bg-white p-5 shadow-sm">
        <div><h1 className="text-2xl font-black text-[#1C1917]">????? ?????</h1><p className="mt-1 text-sm text-[#A8A29E]">{rates.length} ??????</p></div>
      ...
      <div className="overflow-hidden rounded-2xl border border-[#E5E0D8] bg-white shadow-sm">
        ...
        :rates.length===0?<p className="p-10 text-center text-sm text-[#A8A29E]">?? ???? ????? ???</p>
        :(
      ...
      {selected&&(
        <Modal title={selected.governorate} onClose={()=>setSelected(null)}>
          ...
          <div className="space-y-4">
            ...
            <button onClick={save} disabled={saving} className="w-full rounded-xl bg-[#B8860B] py-2.5 text-sm font-bold text-white hover:bg-[#9A7209] disabled:opacity-50">{saving?'...':'??? ?????????'}</button>
          </div>
        </Modal>
      )}
    </div>
